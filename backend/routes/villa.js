...
router.post('/save', async (req, res) => {
  ...
  const villa = new Villa({ villaName, address, spaces, users: [userId], villaId });
...
router.get('/user-villas', async (req, res) => {
  ...
  try {
    const villas = await Villa.find({ users: userId });
...
//   try {
//     const villa = await Villa.findOneAndDelete({ _id: villaId, users: userId });
//     if (!villa) {
...
router.post('/add-villa', async (req, res) => {
  ...
  try {
    ...
    if (!villa.users.includes(userId)) {
      villa.users.push(userId);
      await villa.save();
    }
...
router.delete('/remove-villa/:id', async (req, res) => {
  ...
  try {
    ...
    if (villa.users[0].toString() === userId) {
      await Villa.deleteOne({ _id: villaId });
      return res.status(200).json({ message: 'Villa deleted successfully' });
    }

    // 빌라 목록에서만 제거
    const updatedUsers = villa.users.filter(user => user.toString() !== userId);
